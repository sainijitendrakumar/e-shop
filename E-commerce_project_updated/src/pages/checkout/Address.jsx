...
function Address({checkoutToken,submit,setCurrentState }) {
    ...
    useEffect(() => {
        try {
            if (checkoutToken && shippingSubDivision) fetchShippingOptions(checkoutToken.id, shippingCountry, shippingSubDivision)
        } catch (error) {
            console.log("Error:-", error);
        }
    }, [shippingSubDivision])
                    ...
                    <div className="grid md:grid-cols-2 md:gap-6">
                        <div className="relative z-0 w-full mb-1 group">
                            ...
                            <select id="countries" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                ...
                                {countries !== 0 && countries.map((countries,i) => (
                                    <option key={i} value={countries.id}>{countries.label}</option>
                                ))}
                            </select>
                        </div>
                        <div className="relative z-0 w-full mb-1 group">
